<form action="/governator/master/{{ $model }}/edit/{{ $id }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input value="{{ $data['article']->title }}" type="text" name="title" placeholder="Entrez un titre..." id="title">
    </div>
    <div class="form-group">
        <label for="author">Auteur</label>
        <input value="{{ $data['article']->author }}" type="text" name="author" placeholder="Personne connectée" id="author">
    </div>
    <div class="form-group">
        <label for="article_ckeditor">Contenu de l'article</label>
        <textarea id="article_ckeditor" name="content">{{ $data['article']->content }}</textarea>
    </div>
    <div class="form-group">
