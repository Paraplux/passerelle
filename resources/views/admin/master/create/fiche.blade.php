<form name="fiche" action="/governator/manager/fiche/add" method="POST">
    ...
    <div class="form-group">
        ...
        <input value="{{ old('name') }}" type="text" name="name" placeholder="Entrez le libellé de l'action..." id="name">
    </div>
    <div class="form-group keyword">
        ...
        <input class="old-keyword-input" value="{{ old('keyword-input') }}" type="hidden">
    ...
    <div class="form-group">
        ...
        <textarea name="content" id="fiche_ckeditor_1">{{ old('content') }}</textarea>
    </div>
    <div class="form-group">
        <label for="fiche_ckeditor_2">Programme</label>
        <textarea name="program" id="fiche_ckeditor_2">{{ old('program') }}</textarea>
    </div>
    <div class="form-group">
            <label for="fiche_ckeditor_3">Outils</label>
            <textarea name="tools" id="fiche_ckeditor_3">{{ old('tools') }}</textarea>
        </div>
    <div class="form-group">
        <label for="fiche_ckeditor_4">Pré requis</label>
        <textarea name="pre_requisite" id="fiche_ckeditor_4">{{ old('pre_requisite') }}</textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <input value="{{ old('certification') }}" placeholder="Titre professionnel, Attestation de fin de formation, ..." type="text" name="certification" id="certification">
    </div>
    <div class="form-group">
        <div class="form-small-group">
            <span><label for="date">Début</label><input value="{{ old('date_start') }}" type="date" name="date_start" id="date"></span>
